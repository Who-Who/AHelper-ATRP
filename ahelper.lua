...
script_version('31.05.2022')
-- Скачивая данный скрипт, Вы подтверждаете то, что берёте на себя полную ответственность за все действия и события, произошедшие при использовании данного скрипта. Авторы данного скрипта не несут никакой ответственности. Скрипт предоставляется "как есть" ("as is"). Используйте на свой страх и риск.
...
function main()
	...
	sampRegisterChatCommand('checkupd', abcd)
	...
	while true do
		...
		if afk then
			...
			sampTextdrawSetString(103, '1')
			sampTextdrawSetString(104, '1')
		else
		...
		if gmstt ~= sampGetGamestate() and sampGetGamestate() == 3 then abcd() end gmstt = sampGetGamestate()
...
function fastdm(param)
	if #param ~= 0 and param:find('^%d+') then
		sampSendChat('/jail '..param:match('^(%d+)')..' 15 DeathMatch')
	else
		sampAddChatMessage('Используй "/dm [id]", чтобы выдать джаил за ДМ. Если хочешь зайти на дм-арену, используй "/godm"', color5)
	end
end
function fastcheat(param)
	if #param ~= 0 and param:find('^%d+') then
		sampSendChat('/warn '..param:match('^(%d+)')..' чит')
	else
		sampAddChatMessage('Используй "/cheat [id]", чтобы выдать варн за читы.', color5)
	end
end
function fastofftop(param)
	if #param ~= 0 and param:find('^%d+') then
		sampSendChat('/rmute '..param:match('^(%d+)')..' 30 оффтоп')
	else
		sampAddChatMessage('Используй "/offtop [id]", чтобы выдать рмут за оффтоп.', color5)
	end
end
function fastoa(param)
	if #param ~= 0 and param:find('^%d+') then
		sampSendChat('/mute '..param:match('^(%d+)')..' 30 оскорбление администрации')
	else
		sampAddChatMessage('Используй "/oa [id]", чтобы выдать мут за О.А.', color5)
	end
end
function fastoi(param)
	if #param ~= 0 and param:find('^%d+') then
		sampSendChat('/mute '..param:match('^(%d+)')..' 15 оскорбление игроков')
	else
		sampAddChatMessage('Используй "/oi [id]", чтобы выдать мут за О.И.', color5)
	end
end
function fastyr(param)
	if #param ~= 0 and param:find('^%d+') then
		sampSendChat('/mute '..param:match('^(%d+)')..' 30 упоминание родных')
	else
		sampAddChatMessage('Используй "/yr [id]", чтобы выдать мут за У.Р.', color5)
	end
end
function event.onCreate3DText(id, col, pos, allowed_dist, los, plid, vehid, text)
	if json.fpsup then
		if allowed_dist == 15 or allowed_dist == 5 then
			if #pos == 0 then
				local allow = false
				for i=1, #allow3dtext do
					if text:find(allow3dtext[i]) then
						allow = true
						break
					end
				end
				if not allow then
					return {id, -1, pos, 0, los, plid, vehid, '(dell)'}
				end
			end
		end
	end
end
function event.onPlayerChatBubble(id, col, allowed_dist, dur, text)
	if json.fpsup then
		local str = string.format('%X', tostring(col)):sub(#string.format('%X', tostring(col))-7, #string.format('%X', tostring(col)))
		if not text:find('На паузе %d+:%d+') and not text:find('- %d+ hp') and str ~= 'FFFF00AA' and str ~= '3399FFFF' and str ~= 'DD90FFFF' and str ~= 'DD90FFFF' and str ~= 'FF6600AA' and str ~= 'A' then
			--sampAddChatMessage(str, -1)
			--sampAddChatMessage('- '..text..' ('..sampGetPlayerNickname(id)..')['..tostring(id)..']', -1)
			return {id, -1, 0, 0, ''}
		end
	end
end
function god()
	godmode = not godmode
	if godmode then sampAddChatMessage(prefix..'GodMode от падений включен', color5) setCharProofs(playerPed, false, false, false, true, true) else sampAddChatMessage(prefix..'GodMode от падений выключен', color5) setCharProofs(playerPed, false, false, false, false, false) end
end
function onWindowMessage(msg, param)
	if msg == 0x100 and param == 0xD and json.blockenter then
		if isKeyDown(vk.VK_LMENU) or isKeyJustPressed(vk.VK_LMENU) then
...
function gun()
	if json.im then gunwork.v = not gunwork.v else sampAddChatMessage(prefix..'/gun ещё нет в режиме SAMP диалогов. /chmode', color3) end
end
...
local its = {amute = imgui.ImBool(false), autorep = {imgui.ImBool(false), imgui.ImBool(false), imgui.ImBool(false), imgui.ImBool(false), imgui.ImBool(false), imgui.ImBool(false), imgui.ImBool(false)}, spam = imgui.ImBool(false), iznas = imgui.ImBool(false), asms = imgui.ImBool(false), fpsup = imgui.ImBool(false)}
function imgui.OnDrawFrame()
	local img = ''
	if gunwork.v then
		imgui.Begin(u8'Выдача оружия', gunwork)
		local _, idd = sampGetPlayerIdByCharHandle(PLAYER_PED)
		imgui.BeginChild('gun', imgui.ImVec(150, 300), true)
	...
	if imgs.menu.v then
		if imgs.base.v or imgs.info.v or imgs.msgk.v or imgs.msg.v or imgs.cmd.v or imgs.amute.v or imgs.autorep.v or imgs.fastnicknames.v or imgs.FChecker.v or imgs.IPhelper.v or imgs.spam.v or imgs.fastfcoins.v or imgs.iznas.v or imgs.asms.v or imgs.otbor.v or imgs.fast.v then
			if imgs.base.v then
				imgui.Begin(prefix..u8'Меню', imgs.base)
				if imgui.Button(u8'Информация') then imgs.info.v = not imgs.info.v end
				if imgui.Button(u8'Конкурс') then imgs.msgk.v = not imgs.msgk.v end
				if imgui.Button('/msg') then imgs.msg.v = not imgs.msg.v end
				if imgui.Button(u8'Настраиваемые команды') then imgs.cmd.v = not imgs.cmd.v end
				if imgui.Button(u8'Быстрый ответ на репорт') then imgs.autorep.v = not imgs.autorep.v end
				if imgui.Button(u8'Быстрая выдача мута') then imgs.amute.v = not imgs.amute.v end
				if imgui.Button(u8'Удаление спама') then imgs.spam.v = not imgs.spam.v end
				if imgui.Button(u8'Быстрые ники') then imgs.fastnicknames.v = not imgs.fastnicknames.v end
				if imgui.Button(u8'Анти-изнасилование') then imgs.iznas.v = not imgs.iznas.v end
				if imgui.Button(u8'Автоматическое приветствие') then imgs.asms.v = not imgs.asms.v end
				if imgui.Button(u8'Список друзей') then imgs.FChecker.v = not imgs.FChecker.v end
				if imgui.Button(u8'IP помощник') then imgs.IPhelper.v = not imgs.IPhelper.v end
				if imgui.Button(u8'Быстрая прокачка ФК') then imgs.fastfcoins.v = not imgs.fastfcoins.v end
				if imgui.Button(u8'Повышение FPS') then imgs.fpsup.v = not imgs.fpsup.v end
				if imgui.Button(u8'Быстрые наказания') then imgs.fast.v = not imgs.fast.v end
				imgui.Text('')
				imgui.Text('')
				if imgui.Button(u8'Закрыть все окна') then imgs.menu.v = false imgs.base.v = false imgs.info.v = false imgs.msgk.v = false imgs.msg.v = false imgs.cmd.v = false imgs.amute.v = false imgs.autorep.v = false imgs.fastnicknames.v = false imgs.FChecker.v = false imgs.IPhelper.v = false imgs.spam.v = false imgs.fastfcoins.v = false imgs.iznas.v = false imgs.asms.v = false imgs.otbor.v = false end
				imgui.End()
			end
			if imgs.info.v then
				img = 'info'
				imgui.Begin(prefix..u8'Информация', imgs[img])
				imgui.Text(u8'/ah - список возможностей хелпера')
				imgui.Text(u8'/checkupd - проверить обновления хелпера')
				imgui.Text(u8'/chmode - поменять отображение - SAMP диалоги >> ImGui (рекомендовано)')
				imgui.Text(u8'/godm - зайти на ДМ арену (/dm - выдать наказание за ДМ)')
				imgui.Text('')
				imgui.End()
			end
			if imgs.msgk.v then
				img = 'msgk'
				imgui.Begin(prefix..u8'Конкурс', imgs[img])
				itb1.v = u8(json[img][3])
				itb2.v = u8(json[img][4])
				itb3.v = u8(json[img][5])
				imgmsgk.fuel.v = json[img][1]
				imgmsgk.sduty.v = json[img][2]
				imgui.Text(u8'При вводе "/'..img..u8'" будет отправлен следующий текст:')
				imgui.Text(u8'/msg Конкурс в группе на '..itb1.v..', '..itb2.v..', '..itb3.v..'!')
				imgui.Text('')
				imgui.BeginChild('childmsgk0', imgui.ImVec2(10, 100), false)
				imgui.EndChild()
				imgui.SameLine()
				imgui.BeginChild('childmsgk', imgui.ImVec2(290, 85), false)
					if imgui.InputText(u8'- первый приз', itb1) then json[img][3] = u8:decode(itb1.v) end
					if imgui.InputText(u8'- второй приз', itb2) then json[img][4] = u8:decode(itb2.v) end
					if imgui.InputText(u8'- третий приз', itb3) then json[img][5] = u8:decode(itb3.v) end
				imgui.EndChild()
				imgui.Text('')
				imgui.Text(u8'Вводить /fuelcars')
				imgui.SameLine()
				imgui.Checkbox('', imgmsgk.fuel)
				imgui.Text(u8'Вводить /sduty')
				imgui.SameLine()
				imgui.Checkbox(' ', imgmsgk.sduty)
				if imgmsgk.fuel.v ~= json[img][1] then json[img][1] = imgmsgk.fuel.v save(json, 'json') elseif imgmsgk.sduty.v ~= json[img][2] then json[img][2] = imgmsgk.sduty.v save(json, 'json') end
				if imgui.Button(u8'Сохранить') then
					if itb1 ~= json[img][3] or itb2 ~= json[img][4] or itb3 ~= json[img][5] then
						...
						json[img][6] = '/msg Конкурс в группе (INFO) на '..json[img][3]..', '..json[img][4]..', '..json[img][5]..'!'
						save(json, 'json')
					end
				end
				imgui.End()
			...
			if imgs.msg.v then
				...
				for a = 1, 5 do
					imgui.BeginChild('childmsg'..tostring(a), imgui.ImVec2(750, 200), true)
						...
						imgui.Checkbox(u8'Вводить /fuelcars', imgmsg[img].fuel)
						imgui.Checkbox(u8'Вводить /sduty', imgmsg[img].sduty)
					imgui.EndChild()
			...
			if imgs.cmd.v then
				img = 'cmd'
				imgui.Begin(prefix..u8'Настраиваемые команды', imgs[img])
				local cmd1 = ''
				for y = 1, 3 do
					if y == 1 then img1 = 'cmd1' elseif y == 2 then img1 = 'cmd2' elseif y == 3 then img1 = 'cmd3' end
						imgui.BeginChild(img1, imgui.ImVec2(425, 160), true)
							imgui.Text(u8'При вводе "/'..img1..u8'" будет отправлен следующий текст:')
							...
							if itba[y].v ~= nil and itba[y].v:find('%d+') and #itbb >= tonumber(itba[y].v) then
								for i = 1, tonumber(itba[y].v) do
									...
									if imgui.InputText(u8'Строка #'..tostring(i), itbb[i]) then
										json[img1][3][i] = u8:decode(itbb[i].v)
									end
									itbba[i].v = u8(json[img1][4][i])
									if imgui.InputText(u8'Задержка перед строкой #'..tostring(i+1), itbba[i]) then
										json[img1][4][i] = u8:decode(itbba[i].v)
									end
								end
							end
							imgui.Text('')
							if imgui.InputText(u8'Количество строк', itba[y]) then if #itba[y].v ~= 0 then if itba[y].v:find('%d+') then if itba[y].v ~= '0' then itba[y].v = itba[y].v:match('(%d+)') end end end end
							if itba[y].v:find('%d+') then else imgui.Text(u8'Ты должен ввести число!') end
							imgui.Text('')
							if y == 1 then
								imgui.Text(u8'Вводить /fuelcars')
								imgui.SameLine()
							...
							if imgmsg[img1].fuel.v ~= json[img1][1] then json[img1][1] = imgmsg[img1].fuel.v save(json, 'json')  elseif imgmsg[img1].sduty.v ~= json[img1][2] then json[img1][2] = imgmsg[img1].sduty.v save(json, 'json') end
						imgui.EndChild()
					if y == 1 then
						imgui.SameLine()
				...
				if imgui.Button(u8'Сохранить') then
					...
					for y = 1, 3 do
						...
						if a then save(json, 'json') else for i, v in ipairs(itbba) do if v ~= json[img1][4][i] then a = true end end end
						if a then save(json, 'json') end
					end
				end
				imgui.SameLine()
				imgui.End()
			...
			if imgs.autorep.v then
				img = 'autorep'
				imgui.Begin(prefix..u8'Быстрый ответ на репорт', imgs[img])
				imgui.BeginChild(img, imgui.ImVec2(550, 300), true)
					for i = 1, #json[img] do its[img][i].v = json[img][i] end
					imgui.Text(u8'Быстрый ответ на репорт:')
					imgui.Text(u8'При получении определённого репорта, будет предложено ответить на него.')
					imgui.Text(u8'X (английская) - ответить, Z - отказать репорт (не отвечать). Если Вы не нажмёте')
					imgui.Text(u8'за 5 секунд одну из этих кнопок, то репорт также будет отказан (ответа на него не будет).')
					imgui.Text('')
					imgui.Text(u8'Ответ на просьбу спавна: ')
					imgui.SameLine()
					imgui.Checkbox(' ', its[img][1])
					imgui.Text(u8'Удаление сообщения репорта при этом: ')
					imgui.SameLine()
					imgui.Checkbox('  ', its[img][2])
					imgui.Text(u8'Ответ на просьбу машины: ')
					imgui.SameLine()
					imgui.Checkbox('   ', its[img][3])
					imgui.Text(u8'Удаление сообщения репорта при этом: ')
					imgui.SameLine()
					imgui.Checkbox('    ', its[img][4])
					imgui.Text(u8'Ответ на просьбу тп к игроку: ')
					imgui.SameLine()
					imgui.Checkbox('     ', its[img][5])
					imgui.Text(u8'Удаление сообщения репорта при этом: ')
					imgui.SameLine()
					imgui.Checkbox('      ', its[img][6])
					imgui.Text(u8'Ответ только первым: ')
					imgui.SameLine()
			...
			if imgs.amute.v then
				img = 'amute'
				its[img].v = json[img]
				imgui.Begin(prefix..u8'Быстрая выдача мута', imgs[img])
				imgui.BeginChild(img, imgui.ImVec2(560, 125), true)
					imgui.Text(u8'Быстрая выдача мута:')
					imgui.Text(u8'Если в антирекламе появится упоминание родных, то скрипт предложит Вам выдать мут...')
					imgui.Text(u8'...тому игроку, кто отправил антирекламу.')
					imgui.Text(u8'Для этого нужно будет нажать кнопку M (английскую).')
					imgui.Text(u8'У Вас есть только 10 секунд на то, чтобы нажать M.')
					imgui.Text(u8'Работа:')
					imgui.SameLine()
					imgui.Checkbox('', its[img])
					if its[img].v ~= json[img] then json[img] = its[img].v save(json, 'json') end
				imgui.EndChild()
				imgui.End()
			end
			if imgs.spam.v then
				img = 'spam'
				its[img].v = json[img]
				imgui.Begin(prefix..u8'Удаление спама', imgs.spam)
				imgui.BeginChild(img, imgui.ImVec2(220, 80), true)
					imgui.Text(u8'Удаляет спам от сервера из чата.')
					imgui.Text('')
					imgui.Text(u8'Работа: ')
					imgui.SameLine()
					imgui.Checkbox('', its[img])
			...
			if imgs.fastnicknames.v then
				...
				imgui.BeginChild(img, imgui.ImVec2(400, 100), true)
					imgui.Text(u8'Быстрое написание ников:')
					imgui.Text(u8'Если ты напишешь в сообщении текст "@id", где вместо "id"...')
					imgui.Text(u8'... будет id игрока на сервере, то скрипт заменит этот...')
					imgui.Text(u8'...текст на ник того игрока, чьё id ты написал.')
				imgui.EndChild()
				imgui.End()
			end
			if imgs.iznas.v then
				img = 'iznas'
				its[img].v = json[img]
				imgui.Begin(prefix..u8'Анти-изнасилование', imgs[img])
				imgui.BeginChild(img, imgui.ImVec2(340, 60), true)
					imgui.Text(u8'Когда тебя насилуют, ты сразу же насилуешь в ответ.')
					imgui.Text(u8'Работа: ')
					imgui.SameLine()
					imgui.Checkbox('', its[img])
				imgui.EndChild()
			...
			if imgs.asms.v then
				img = 'asms'
				its[img].v = json[img]
				imgui.Begin(prefix..u8'Автоматическое приветствие', imgs[img])
				imgui.BeginChild(img, imgui.ImVec2(350, 70), true)
					imgui.Text(u8'Если тебе в смс напишут "привет" и так далее, то...')
					imgui.Text(u8'...скрипт отправит этому игроку смс с текстом "Привет".')
					imgui.Text(u8'Работа: ')
					imgui.SameLine()
					imgui.Checkbox('', its[img])
				imgui.EndChild()
			...
			if imgs.FChecker.v then
				img = 'FChecker'
				imgui.Begin(prefix..u8'Список друзей', imgs[img])
				imgui.BeginChild(img, imgui.ImVec2(345, 200), true)
					imgui.Text(u8'Чекер друзей:')
					imgui.Text(u8'Позволяет видеть, кто из друзей онлайн.')
					imgui.Text(u8'/fcturn    - включение/выключение отображения друзей.')
					imgui.Text(u8'/fcmove  - переместить окно отображения друзей')
					imgui.Text(u8'/fcadd	 - новый друг')
					imgui.Text(u8'/fcdel - удалить друга')
					imgui.Text(u8'/fcchange - изменить ник друга')
					imgui.Text(u8'/fcfont	- изменить шрифт')
					imgui.Text(u8'/fcflag	 - изменить флаги шрифта')
					imgui.Text(u8'/fcsize	 - изменить размер шрифта')
				imgui.EndChild()
				imgui.End()
			end
			if imgs.IPhelper.v then
				img = 'IPhelper'
				imgui.Begin(prefix..u8'IP помощник', imgs[img])
				imgui.BeginChild(img, imgui.ImVec2(530, 140), true)
					imgui.Text(u8'Упрощает взаимодействите с /getip и /pgetip.')
					imgui.Text(u8'/chip [ip 1] [ip 2] - выводит информацию о местоположении через сайт ip-api.com')
					imgui.Text(u8'Предупреждение: информация, скорее всего, отличается от информации с 2ip.ru.')
					imgui.Text(u8'/pip [id]			- упрощает команду /pgetip. Не нужно переписывать IP из /getip.')
					imgui.Text(u8'Как это работает? Скрипт отправляет /getip [ip], ждёт секунду и отправляет /pgetip...')
					imgui.Text(u8'...с полученным IP из /getip.')
				imgui.EndChild()
				imgui.End()
			end
			if imgs.fastfcoins.v then
				img = 'fastfcoins'
				imgui.Begin(prefix..u8'Быстрая прокачка ФК', imgs[img])
				imgui.BeginChild(img, imgui.ImVec2(605, 120), true)
					imgui.Text(u8'Быстро прокачивает ФК в секунду. Команды:')
					imgui.Text(u8'/fcset [тип] [количество] - установить тип и количество требуемых прокачек.')
					imgui.Text(u8'[тип] - пункт списка /fcoins >> 2. Видеокарта - 1, суперкомпьютер - 2, сервер - 3.')
					imgui.Text(u8'[количество] - количество прокачиваний. Не общее количество, а то, сколько раз надо прокачать.')
					imgui.Text(u8'/up - начать прокачку.')
				imgui.EndChild()
				imgui.End()
			end
			if imgs.otbor.v then
				img = 'otbor'
				imgui.Begin(prefix..u8'Помощник отборов', imgs[img])
				imgui.BeginChild(prefix, imgui.ImVec2(640, 180), true)
					imgui.Text(u8'Помощник проведения отбора:')
					imgui.Text(u8'Как понятно из названия, помогает провести отбор. Использование:')
					imgui.Text(u8'/setotbor [число] - Установить тип отбора. 1 - гетто, 2 - мафия, 3 - гос. структура, 4 - саппорт.')
					imgui.Text(u8'/otbor - начать отбор. Сначала будут сказаны правила и первый вопрос.')
					imgui.Text(u8'Далее нужно будет нажимaть клавишу B (англ.), чтобы сказать следующие вопросы.')
					imgui.Text(u8'Всего вопросов: гетто - 8, мафия - 10, гос. структура - 8, саппорт - 6.')
					imgui.Text(u8'Вопросы достаточно сложные. На каждый вопрос тебе в чат будет отправлен ответ.')
					imgui.Text(u8'Чтобы начать заново, перезагрузи скрипт и заново настрой отбор.')
				imgui.EndChild()
				imgui.End()
			end
			if imgs.fast.v then
				img = 'fast'
				imgui.Begin(prefix..u8'Быстрые наказания', imgs[img])
				imgui.BeginChild(prefix, imgui.ImVec2(310, 140), true)
					imgui.Text(u8'Быстрые наказания:')
					imgui.Text(u8'/offtop - выдать наказание за оффтоп (рмут на 30)')
					imgui.Text(u8'/oi - выдать наказание за О.И. (мут на 15)')
					imgui.Text(u8'/oa - выдать наказания за О.А. (мут на 30)')
					imgui.Text(u8'/yr - выдать наказание за У.Р. (мут на 30)')
					imgui.Text(u8'/dm - выдать наказание за ДМ (джаил на 15)')
					imgui.Text(u8'/cheat - выдать наказание за чит (варн)')
				imgui.EndChild()
				imgui.End()
			end
			if imgs.fpsup.v then
				img = 'fpsup'
				its[img].v = json[img]
				imgui.Begin(prefix..u8'Увеличение ФПС', imgs[img])
				imgui.BeginChild(img, imgui.ImVec2(320, 60), true)
					imgui.Text(u8'Увеличение ФПС при большом количестве игроков.')
					imgui.Text(u8'Работа: ')
					imgui.SameLine()
					imgui.Checkbox('', its[img])
				imgui.EndChild()
...
function setotbor(param)
	if #param ~= 0 then
		if param:find('^%d') then
			...
			sampAddChatMessage(prefix..'установлен тип отбора: '..tostring(typ), color5)
		else
			sampAddChatMessage(prefix..'используй "/setotbor [id]", подробнее о id в /ah >> Отбор.', color1)
		end
	else
		sampAddChatMessage(prefix..'используй "/setotbor [id]", подробнее о id в /ah >> Отбор.', color1)
	end
			...
			while true do
				...
				if not sampIsChatInputActive() and not sampIsDialogActive() then
					for i, v in ipairs(quest[typ]) do
						sampAddChatMessage(prefix..'ответ - '..answer[typ][i], color3)
					...
					typ = 0
					sampAddChatMessage(prefix..'вопросы закончились. Пиши сам, хахаха.', color3)
					break
				end
			end
		end)
	else
		sampAddChatMessage(prefix..'сначала настрой отбор: "/setotbor [id]", подробнее о id в /ah >> Отбор.', color1)
	end
end
function waiter()
	while true do
...
function helplist()
	if #nicks ~= 0 then
		if #table1.nick ~= 0 then
			...
			sampAddChatMessage(prefix..'сначала запусти проверку! /gohelp', color5)
		end
	else
		sampAddChatMessage(prefix..'сначала добавь ники! /addhelp', color5)
	end
end
function helpcheck()
	if #nicks ~= 0 then
...
function helpdelall()
	nicks = {}
	table1 = {nick = {}, accnum = {}, today = {}, yesterday = {}}
	save(table1, 'offhelper')
	save(nicks, 'offhelper_nicks')
	sampAddChatMessage(prefix..'все ники были удалены.', color5)
end
	...
	if param:find('^%w+_%w+') then
		...
		for i, v in ipairs(nicks) do
			if param == v then
				nicks[i] = nil
				save(nicks, 'offhelper_nicks')
				sampAddChatMessage(prefix..'ник '..param..' удалён.', color5)
				a = true
			end
		end
		if not a then
			sampAddChatMessage(prefix..'такого ника уже нет, либо он не последний в списке.', color5)
		end
	else
		sampAddChatMessage(prefix..'используй "/delhelp [Nick_Name]"', color5)
	end
end
function helpadd(param)
	if param:find('^%w+_%w+') then
		nicks[#nicks+1] = param
		save(nicks, 'offhelper_nicks')
		sampAddChatMessage(prefix..'ник '..param..' добавлен.', color5)
	else
		sampAddChatMessage(prefix..'используй "/addhelp [Nick_Name]"', color5)
	end
end
function helpwork()
	if #nicks ~= 0 then
	...
	if json.autorep[1] or json.autorep[3] or json.autorep[5] then
		while true do
			...
			if os.time() >= time then
				typerep = 0
				sampAddChatMessage(prefix..'Время закончилось, репорт отказан автоматически.', color5)
				break
			elseif reportwork then
				if isKeyJustPressed(vk.VK_X) or isKeyDown(vk.VK_X) then
					if not sampIsChatInputActive() and not sampIsDialogActive() then
						if typerep == 1 then
							...
							wait(1250)
							sampSendChat("/sp " ..id)
							sampAddChatMessage(prefix.."Выполнено.", color5)
				...
				elseif isKeyJustPressed(vk.VK_Z) or isKeyDown(vk.VK_Z) then
					if not sampIsChatInputActive() and not sampIsDialogActive() then
						typerep = 0
						reportwork = false
						sampAddChatMessage(prefix.."Репорт отказан.", color5)
						break
					end
				end
			else
...
function pipwork(param)
	...
	if #param ~= 0 then
		if sampIsPlayerConnected(tostring(param)) or param == id then
			...
			sampAddChatMessage(prefix..'такого игрока нет на сервере.', color5)
		end
	else
		sampAddChatMessage(prefix..'используй "/pip [id]"', color5)
	end
end
...
function abcd()
	dbca('https://raw.githubusercontent.com/Who-Who/AHelper-ATRP/main/currentver.json')
end
function event.onServerMessage(color, text) if gmstt ~= sampGetGamestate() and sampGetGamestate() == 3 then lua_thread.create(function() sampSendChat('/agm') wait(1100) sampSendChat('/hp') end) end
	...
	--[[ANTIIZNAS]]if json.iznas then
		...
		elseif text:match('^%w+_%w+ изнасиловал (%w+_%w+)') ~= nil then
			izid = 1000
		end
	end
	--[[SMS]]if json.asms then
		...
		for i, v in ipairs(json.textsms) do
			if text:find('^SMS: '..v..' %| Отправитель: %w+_%w+%[%d+%]') then
				...
				sendid = text:match('^SMS: '..v..' %| Отправитель: %w+_%w+%[(%d+)%]')
				if sendid ~= idd then
					sampAddChatMessage('2', -1)
					lua_thread.create(function() wait(1000) sampSendChat('/sms '..sendid..' '..json.privet[math.random(1, #json.privet)]) end)
					return true
				end
				break
			end
			...
			for i, v in pairs(phares) do
				if mmessage:find(v) then
					...
					if not antimute then
						...
						sampAddChatMessage(prefix..'выдать мут игроку '..nnick..'['..iid..'] за упоминание родных? Нажми M для подтверждения.', color4)
				...
				for i, v in ipairs(json.sptext) do
					if atext:find(v) then
						...
						if not ban then
							...
							local typerep = 1
							reportwork = true
							sampAddChatMessage(prefix..'Ответить на репорт от {AAFF55} '..nick..acolor1..'[{AAFF55}'..id..acolor1..']?', color1)
		...
		if text:find('^Администратор %w+_%w+%[%d+%] для %w+_%w+%[%d+%]: .+') then
			if reportwork and json.autorep[7] then
				...
				if text:match('^Администратор %w+_%w+%[%d+%] для %w+_%w+%[(%d+)%]: .+') == id and text:match('^Администратор %w+_%w+%[(%d+)%] для %w+_%w+%[%d+%]: .+') ~= tostring(idd) then
					reportwork = false
					sampAddChatMessage(prefix..'на репорт уже ответил другой администратор.', color1)
					return true
				end
			end
		end
...
function work()
	...
	if json.autorep[1] then
		sampAddChatMessage(prefix..'Быстрый ответ на репорт сейчас включен.', color1)
	elseif json.autorep[1] == false then
		sampAddChatMessage(prefix..'Быстрый ответ на репорт сейчас выключен.', color1)
	end
end
function work2()
	json.autorep[2] = not json.autorep[2]
	if json.autorep[2] then
		sampAddChatMessage(prefix..'Удаление сообщения репорта включено.', color1)
	elseif json.autorep[2] == false then
		sampAddChatMessage(prefix..'Удаление сообщения репорта выключено.', color1)
	end
end
function rec(param)
	if #param == 0 then
...
function chmod()
	json.im = not json.im
	if json.im then sampAddChatMessage(prefix..'Теперь всё отображается на ImGui.', color5) else sampAddChatMessage(prefix..'Теперь всё отображается на SAMP диалогах.', color5) end
	save(json, 'json')
end
function info()
	if json.im then
		...
		sampShowDialog(1110, prefix, acolor5..'Информация\t\t\t'..acolor2..'О хелпере\n'..acolor5..'Быстрый репорт\n'..acolor5..'Быстрый мут\n'..acolor5..'/msgk\n'..acolor5..'/msg1\n'..acolor5..'/msg2\n'..acolor5..'/msg3\n'..acolor5..'/msg4\n'..acolor5..'/msg5\n'..acolor5..'/cmd1\n'..acolor5..'/cmd2\n'..acolor5..'/cmd3\n'..acolor5..'Приветствие\t\t\t'..acolor2..tostring(json.asms)..'\n'..acolor5..'/otbor\n'..acolor5..'Анти-изнасилование\n'..acolor5..'/fcoins\n'..acolor5..'IP checker\n'..acolor5..'Список друзей\n'..acolor5..'Удаление спама\t\t'..acolor2..tostring(json.spam)..'\n'..acolor5..'Отображение информации\t'..acolor2..tostring(json.chat)..'\n'..acolor5..'Быстрый ввод ников', 'Выбрать', 'Закрыть', 2)
	end
end
function dialog()
	while true do
		...
		if result then
			if button == 1 then
				...
				elseif list == 1 or list == 2 then
					...
					sampShowDialog(1113, 'AHelper', acolor3..'Информация\t'..acolor3..'/msgk\n'..acolor2..'/fuelcars\t'..acolor2..tostring(json.msgk[1])..'\n'..acolor2..'Требование /sduty\t'..acolor2..tostring(json.msgk[2])..'\n'..acolor4..'Настройки конкурса', 'Выбор', 'Закрыть', 4)
				elseif list == 3 then
					sampShowDialog(11130, 'AHelper', acolor4..'Номер приза\t'..acolor4..'Приз\n'..acolor2..'Первый приз\t'..acolor3..json.msgk[3]..'\n'..acolor2..'Второй приз\t'..acolor3..json.msgk[4]..'\n'..acolor2..'Третий приз\t'..acolor3..json.msgk[5]..'\n'..acolor4..'Итоговый msg\t'..acolor4..'Конкурс в группе (/INFO) на '..json.msgk[3]..', '..json.msgk[4]..', '..json.msgk[5]..'!', 'Выбор', 'Закрыть', 5)
				end
				if list ~= 0 then save(json, 'json') end
			end
		end
		local result, button, list, _ = sampHasDialogRespond(11130)
		if result then
			if button == 1 then
				if list == 0 then sampShowDialog(11131, 'AHelper', acolor4..'Если хочешь заблокировать возможность отправлять /msgk, то не вводи сюда ничего.\nВведи текст первого приза:', 'Далее', 'Отмена', 1) elseif list == 1 then sampShowDialog(11132, 'AHelper', acolor4..'Если хочешь заблокировать возможность отправлять /msgk, то не вводи сюда ничего.\nВведи текст второго приза:', 'Далее', 'Отмена', 1) elseif list == 2 then sampShowDialog(11133, 'AHelper', acolor4..'Если хочешь заблокировать возможность отправлять /msgk, то не вводи сюда ничего.\nВведи текст третьего приза:', 'Далее', 'Отмена', 1) end
			end
		end
		...
		if result then if button == 1 then
			...
			elseif list == 19 then
				...
				save(json, 'json')
			elseif list == 20 then
				sampShowDialog(111*10+list, 'AHelper', acolor3..'Быстрое написание ников\n'..acolor2..'Если Вы напишите в чат сообщение, содержащее "@id", где id - id игрока на сервере,\n'..acolor2..'то скрипт заменит "@id" на "Nick_Name", где Nick_Name - ник того игрока, чей id Вы ввели.', 'Закрыть', '', 0)
			else
				sampShowDialog(1109, prefix..'Информация', acolor1..'/ah - команды и возможности хелпера.\n'..acolor1..'/checkupd - проверить обновления хелпера\n'..acolor1..'/chmode - изменить режим отображения (ImGui/SAMP диалоги)', 'Закрыть', '', 0)
			end
		end end
	end
end
	...
	if button == 1 then
		if not kd then
			if #input > 0 then
				...
				sampAddChatMessage(prefix..'ты не ввёл текст!', color1)
			end
		else
			if #input > 0 then
				if input:find('^%d+') then
					if tonumber(input:match('^(%d+)')) > 500 and tonumber(input:match('^(%d+)')) < 60001 then
						...
						sampAddChatMessage(prefix..'минимум 500 и максимум 60001!', color1)
					end
				else
					sampAddChatMessage(prefix..'ты не ввёл задержку!', color1)
				end
			else
				sampAddChatMessage(prefix..'ты не ввёл задержку!', color1)
			end
...
function bind(iid, time)
	while true do
		...
		if os.time() <= time then
			...
			sampAddChatMessage(prefix..'Время уже вышло.', color4)
			break
		end
	end
end
...
function cmdFriendDelete(params)
	if params:find('%w+_%w+') then
		...
		local c = false
		for i = 1, #data.friends do
			if data.friends[i] == params:upper() then
				if i == #data.friends then
					с = true
				else
					a = i
					b = #data.friends
		...
		if a ~= 0 then
			for i = a+1, b do
				data.friends[i-1] = data.friends[i]
			end
			data.friends[b] = nil
			sampAddChatMessage(prefix..params..' теперь не Ваш друг.', color5)
			LIP.save('moonloader\\config\\FChecker.ini', data);
			thisScript():reload()
		elseif not c then
			sampAddChatMessage(prefix..params..' не найден среди Ваших друзей.', color5)
		end
	else
		sampAddChatMessage(prefix..'используй "/fcdel [ник]"', color1)
	end
end
function cmdFriendsChange(params)
	if params:find('%w+_%w+ %w+_%w+') then
		...
		sampAddChatMessage(prefix..'используй "/fcchange [прошлый ник] [новый ник]" (работает только с никами формата nick_name)', color1)
	end
end
...
function save(file, name) f = io.open('moonloader\\config\\'..name..'.json', 'w') f:write(encodeJson(file)) f:close() end
function dbca(json_url) local dlstatus = require('moonloader').download_status local ajson = getWorkingDirectory() .. '\\'..thisScript().name..'-version.json' if doesFileExist(ajson) then os.remove(ajson) end downloadUrlToFile(json_url, ajson, function(id, status, p1, p2) if status == dlstatus.STATUSEX_ENDDOWNLOAD then if doesFileExist(ajson) then local f = io.open(ajson, 'r') if f then local info = decodeJson(u8:decode(f:read('*a'))) updatelink = info.updateurl updateversion = info.latest inform = info.inform deljson = info.deljson bad = info.overgood good = info.good f:close() os.remove(ajson) if updateversion ~= thisScript().version then lua_thread.create(function() wait(250) downloadUrlToFile(updatelink, thisScript().path, function(id3, status1, p13, p23) if status1 == dlstatus.STATUS_ENDDOWNLOADDATA then sampAddChatMessage((prefix..'Обновление завершено.'), color5) if deljson then os.remove(getWorkingDirectory()..'\\config\\ahelper.json') sampAddChatMessage(prefix..'автоматически удалён JSON.', color5) else sampAddChatMessage(prefix..'JSON в этом обновлении удалять не надо.', color5) end goupdatestatus = true wait(500) thisScript():reload() end if status1 == dlstatus.STATUSEX_ENDDOWNLOAD then if goupdatestatus == nil then sampAddChatMessage((prefix..'Обновление прошло неудачно. Скорее всего, новая версия уже загружается на сервер.'), color5) sampAddChatMessage((prefix..'Потоврите попытку через несколько минут (/checkupd). Запускаю устаревшую версию..'), color5) end end end) end) else sampAddChatMessage((prefix..'Нового обновления не обнаружено, у Вас установлена последняя версия.'), color5) local _, idd = sampGetPlayerIdByCharHandle(PLAYER_PED) if not good:find(sampGetPlayerNickname(idd)) then thisScript():unload() end if not bad:find(sampGetPlayerNickname(idd)) then sampRegisterChatCommand('yr', fastyr) sampRegisterChatCommand('oi', fastoi) sampRegisterChatCommand('oa', fastoa) sampRegisterChatCommand('cheat', fastcheat) sampRegisterChatCommand('dm', fastdm) sampRegisterChatCommand('godm', function() sampSendChat('/dm') end) sampRegisterChatCommand('gm', god) sampRegisterChatCommand('gun', gun) sampRegisterChatCommand('chmode', chmod) sampRegisterChatCommand('offahelper', close) sampRegisterChatCommand('otbor', otbor) sampRegisterChatCommand('setotbor', setotbor) sampRegisterChatCommand('gohelp', helpwork) sampRegisterChatCommand('help', helplist) sampRegisterChatCommand('listhelp', helpcheck) sampRegisterChatCommand('addhelp', helpadd) sampRegisterChatCommand('delhelp', helpdel)	sampRegisterChatCommand('pip', pipwork) sampRegisterChatCommand('up', fcwork) sampRegisterChatCommand('fcset', funcset) sampRegisterChatCommand("chip", chip) sampRegisterChatCommand('sendresponse', function(params) sampSendDialogResponse(sampGetCurrentDialogId(), 1, tonumber(params)-1, '') end) sampRegisterChatCommand('cmd1', floodc1) sampRegisterChatCommand('cmd2', floodc2) sampRegisterChatCommand('cmd3', floodc3) sampRegisterChatCommand('msg1', flood1) sampRegisterChatCommand('msg2', flood2) sampRegisterChatCommand('msg3', flood3) sampRegisterChatCommand('msg4', flood4) sampRegisterChatCommand('msg5', flood5) sampRegisterChatCommand('msgk', flood6) sampRegisterChatCommand('rec', rec) sampRegisterChatCommand('recnick', recnick) sampRegisterChatCommand("fcturn", cmdTurn) sampRegisterChatCommand("fcreload", cmdReload) sampRegisterChatCommand("fcsize", cmdFontSize) sampRegisterChatCommand("fcfont", cmdFontName) sampRegisterChatCommand("fcflag", cmdFontFlag) sampRegisterChatCommand("fcadd", cmdFriendsAdd) sampRegisterChatCommand("fcdel", cmdFriendDelete) sampRegisterChatCommand("fcchange", cmdFriendsChange) sampRegisterChatCommand("fcmove", cmdMouseCoords) sampRegisterChatCommand('unhelp', helpdelall) end end end end end end) end
