	...
	if imgs.menu.v then
		if imgs.base.v or imgs.info.v or imgs.msgk.v or imgs.msg.v or imgs.cmd.v or imgs.amute.v or imgs.autorep.v or imgs.fastnicknames.v or imgs.FChecker.v or imgs.IPhelper.v or imgs.spam.v or imgs.fastfcoins.v or imgs.iznas.v or imgs.asms.v or imgs.otbor.v then
			...
			if imgs.msgk.v then
				...
				if imgmsgk.fuel.v ~= json[img][1] then json[img][1] = imgmsgk.fuel.v save() elseif imgmsgk.sduty.v ~= json[img][2] then json[img][2] = imgmsgk.sduty.v save() end
			...
			if imgs.cmd.v then
				...
				for y = 1, 3 do
					if y == 1 then img1 = 'cmd1' elseif y == 2 then img1 = 'cmd2' elseif y == 3 then img1 = 'cmd3' end
						imgui.BeginChild(img1, imgui.ImVec2(425, 160), true)
							...
							if imgmsg[img1].fuel.v ~= json[img1][1] then json[img1][1] = imgmsg[img1].fuel.v save() elseif imgmsg[img1].sduty.v ~= json[img1][2] then json[img1][2] = imgmsg[img1].sduty.v save() end
						imgui.EndChild()
					if y == 1 then
						imgui.SameLine()
				...
				if imgui.Button(u8'Сохранить') then
					...
					for y = 1, 3 do
						...
						if a then save() else for i, v in ipairs(itbba) do if v ~= json[img1][4][i] then a = true end end end
						if a then save() end
					end
				end
				imgui.SameLine()
			...
			if imgs.amute.v then
				...
				if its[img].v ~= json[img] then json[img] = its[img].v save() end
				imgui.EndChild()
				imgui.End()
			end
			if imgs.spam.v then
