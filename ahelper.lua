...
local notphares = {'занима', 'пойма', 'поднима', 'понима', 'принима', 'снима', 'слома', 'нажима', 'Занима', 'Пойма', 'Поднима', 'Понима', 'Принима', 'Снима', 'Слома', 'Нажима', 'ЗАНИМА', 'ПОЙМА', 'ПОДНИМА', 'ПОНИМА', 'ПРИНИМА', 'СНИМА', 'СЛОМА', 'НАЖИМА'}
...
function main()
	...
	local a = false
	local acceptednicknames = {'Emmerald_Lacoste', 'Aodhfayonn_Sullivan', 'Orlando_BlackStar', 'Alexandr_Mansory', 'North_Forest', 'John_Sullivan', 'Tenshi_TuathaDe', 'Zahar_TuathaDe', 'Christopher_Young'}
...
apply_custom_style()
local imgs = {menu = imgui.ImBool(false), info = imgui.ImBool(false), msgk = imgui.ImBool(false), msg = imgui.ImBool(false), cmd = imgui.ImBool(false), amute = imgui.ImBool(false), autorep = imgui.ImBool(false), fastnicknames = imgui.ImBool(false), FChecker = imgui.ImBool(false), IPhelper = imgui.ImBool(false), spam = imgui.ImBool(false), fastfcoins = imgui.ImBool(false), iznas = imgui.ImBool(false), asms = imgui.ImBool(false), otbor = imgui.ImBool(false)}
...
function imgui.OnDrawFrame()
	...
	local img = ''
	if imgs.menu.v then
		imgui.Begin(prefix..u8'Меню', imgs.menu)
	...
	if imgs.IPhelper.v then
		...
		imgui.BeginChild(prefix, imgui.ImVec2(530, 160), true)
	...
	if imgs.fastfcoins.v then
		...
		imgui.BeginChild(prefix, imgui.ImVec2(605, 120), true)
	...
	--[[if img:find('msg%d') or img == 'cmd' then
		if imgmsg[img].fuel.v ~= json[img][1] or imgmsg[img].sduty.v ~= json[img][2] then
			json[img][1] = imgmsg[img].fuel.v
			json[img][2] = imgmsg[img].sduty.v
			save()
	...
	--[[if img ~= 'cmd' then imgui.Text('') end
	if imgui.Button(u8'Закрыть') then
		imgs.menu.v = not imgs.menu.v
	end]]
	imgui.Process = imgs.menu.v
end
function close()
...
function autoupdate2()
	...
	function(id3, status1, p13, p23)
		...
		elseif status1 == dlstatus.STATUS_ENDDOWNLOADDATA then
			sampAddChatMessage((prefix..'Обновление завершено! Автоматически удалён JSON.'), color5)
			os.remove(getWorkingDirectory()..'\\config\\ahelper.json')
			goupdatestatus = true
