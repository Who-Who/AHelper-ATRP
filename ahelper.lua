...
local prefix = 'Helper: '
local imgbase = imgui.ImBool(false)
function main()
	...
	if not a then
		sampAddChatMessage(prefix..'произошла непредвиденная ошибка. Код ошибки: {'..acolor1..'}215.', color3)
	...
	sampRegisterChatCommand('cmds', commands)
	sampRegisterChatCommand('helper', info)
	...
	while true do
		...
		if cursorEnabled and not imgbase.v then
...
apply_custom_style()
local infod = imgui.ImBool(false)
...
function imgui.OnDrawFrame()
	if imgbase.v then
		...
		infod.v = json.infod
		if infod.v then
			imgui.SetNextWindowPos(imgui.ImVec2(sw-600, sh-350), nil, imgui.WindowFlags.NoResize+imgui.WindowFlags.NoCollapse)
			imgui.SetNextWindowSize(imgui.ImVec2(600, 350), nil, imgui.WindowFlags.NoResize+imgui.WindowFlags.NoCollapse)
		elseif infod.v == false then
			imgui.SetNextWindowPos(imgui.ImVec2(sw-600, sh-350), imgui.Cond.FirstUseEver)
			imgui.SetNextWindowSize(imgui.ImVec2(600, 350), imgui.Cond.FirstUseEver)
		end
		imgui.Begin(prefix..''..img, imgbase)
		if img == 'info' then
			...
			imgui.Text('')
			imgui.Text(u8'Зафиксированные окна: ')
			imgui.SameLine()
			imgui.Checkbox('', infod)
			if infod.v ~= json.infod then json.infod = infod.v save() end
		elseif img == 'msgk' then
			...
			imgui.Text('')
			imgui.BeginChild('empchild1', imgui.ImVec2(10, 100), false)
			imgui.EndChild()
			imgui.SameLine()
			imgui.BeginChild('child1', imgui.ImVec2(290, 85), false)
		...
		elseif img:find('cmd%d') then
			imgui.Text(u8'При вводе "/'..img..u8'" будет отправлен следующий текст:')
			imgmsg[img].fuel.v = json[img][1]
			imgmsg[img].sduty.v = json[img][2]
			if itba.v ~= nil and itba.v:find('%d+') and #itbb >= tonumber(itba.v) then
				for i = 1, tonumber(itba.v) do
					itbb[i].v = u8(json[img][3][i])
					if imgui.InputText(u8'Строка #'..tostring(i), itbb[i]) then
						json[img][3][i] = u8:decode(itbb[i].v)
					end
					itbba[i].v = u8(json[img][4][i])
					if imgui.InputText(u8'Задержка перед строкой #'..tostring(i+1), itbba[i]) then
						json[img][4][i] = u8:decode(itbba[i].v)
					end
				end
			end
			imgui.Text('')
			if imgui.InputText(u8'Количество строк', itba) then if #itba.v ~= 0 then if itba.v:find('%d+') then if itba.v ~= '0' then itba.v = itba.v:match('(%d+)') end end end end
			if itba.v:find('%d+') then else imgui.Text(u8'Ты должен ввести число!') end
			imgui.Text('')
			imgui.Text(u8'Вводить /fuelcars')
			imgui.SameLine()
			imgui.Checkbox('', imgmsg[img].fuel)
			imgui.Text(u8'Вводить /sduty')
			imgui.SameLine()
			imgui.Checkbox(' ', imgmsg[img].sduty)
			if imgmsg[img].fuel.v ~= json[img][1] then json[img][1] = imgmsg[img].sduty.v save() elseif imgmsg[img].sduty.v ~= json[img][2] then json[img][2] = imgmsg[img].sduty.v save() end
			if imgui.Button(u8'Сохранить') then
				imgbase.v = not imgbase.v
				local a = false
				for i, v in ipairs(itbb) do if v ~= json[img][3][i] then a = true break end end
				if a then save() else for i, v in ipairs(itbba) do if v ~= json[img][4][i] then a = true end end end
				if a then save() end
			end
		elseif img == 'amute' then
			its[img].v = json[img]
		...
		elseif img == 'fast nicknames' then
			imgui.Text(u8'Быстрое написание ников:')
			imgui.Text(u8'Если ты напишешь в сообщении текст "@id", где вместо "id" будет id игрока на сервере, то...')
			imgui.Text(u8'...скрипт заменит этот текст на ник того игрока, чьё id ты написал.')
		elseif img == 'FChecker' then
			imgui.Text(u8'Чекер друзей:')
			imgui.Text(u8'Позволяет видеть, кто из друзей онлайн. Использование:')
			sampRegisterChatCommand("fcturn", cmdTurn)
		    sampRegisterChatCommand("fcsize", cmdFontSize)
		    sampRegisterChatCommand("fcfont", cmdFontName)
		    sampRegisterChatCommand("fcflag", cmdFontFlag)
		...
		elseif img == 'IP helper' then
			imgui.Text(u8'Помощник работы с IP:')
			imgui.Text(u8'Упрощает взаимодействите с /getip и /pgetip. Использование:')
		...
		elseif img == 'spam' then
			its[img].v = json[img]
			imgui.Text(u8'Удаление спама из чата:')
			imgui.Text(u8'Что же этот модуль делает? Удивительно! Удаляет спам из чата.')
			imgui.Text(u8'Спамом считаются сообщения по типу')
			imgui.Text(u8'"Attractive RP | Если вы не добавили наш сервер в избранное ...";')
			imgui.Text(u8'"Администратор Ace_Will: Раздачи донат рублей проходят ..."')
			imgui.Text(u8'И многие другие.')
			imgui.Text('')
			imgui.Text(u8'Работа: ')
			imgui.SameLine()
			imgui.Checkbox('', its[img])
			if its[img].v ~= json[img] then json[img] = its[img].v save() end
		elseif img == 'fast fcoins' then
			imgui.Text(u8'Быстрая прокачка ФК:')
			imgui.Text(u8'Что же этот модуль делает? Удивительно! Быстро прокачивает ФК в секунду. Команды:')
		...
		elseif img == 'iznas' then
			its[img].v = json[img]
			imgui.Text(u8'Анти-изнасилование:')
			imgui.Text(u8'Когда тебя насилуют, ты сразу же насилуешь в ответ. Баг с срабатыванием при /me пофикшен.')
			imgui.Text(u8'Активация автоматическая.')
			imgui.Text(u8'Работа: ')
			imgui.SameLine()
			imgui.Checkbox('', its[img])
			if its[img].v ~= json[img] then json[img] = its[img].v save() end
		elseif img == 'asms' then
			its[img].v = json[img]
			imgui.Text(u8'Автоматическое приветствие:')
			imgui.Text(u8'Всё понятно из названия. Если тебе в смс напишут "привет" и так далее, то скрипт отправит этому...')
			imgui.Text(u8'...игроку смс с текстом "Привет".')
			imgui.Text(u8'Работа: ')
			imgui.SameLine()
			imgui.Checkbox('', its[img])
			if its[img].v ~= json[img] then json[img] = its[img].v save() end
		elseif img == 'otbor' then
		...
		if img:find('msg%d') or img:find('cmd%d') then
...
function setotbor(param)
	if #param ~= 0 then
		if param:find('^%d') then
			...
			sampAddChatMessage(prefix..'используй "/setotbor [id]", подробнее о id в /ahelper >> Отбор.', color1)
		end
	else
		sampAddChatMessage(prefix..'используй "/setotbor [id]", подробнее о id в /ahelper >> Отбор.', color1)
	end
...
function otbor()
	if typ ~= 0 then
		...
		sampAddChatMessage(prefix..'сначала настрой отбор: "/setotbor [id]", подробнее о id в /ahelper >> Отбор.', color1)
	end
end
function waiter()
	while true do
...
function info()
	sampShowDialog(666, prefix..'список возможностей', '{'..acolor1..'}Информация\n{'..acolor5..'}Выдача мута\n{'..acolor5..'}Ответ на репорт\n{'..acolor5..'}Быстрые ники\n{'..acolor5..'}Список друзей\n{'..acolor5..'}Удаление спама\n{'..acolor5..'}Анти-изнасилование\n{'..acolor5..'}Автоматическое приветствие', 'Выбрать', 'Закрыть', 2)
end
function commands()
	sampShowDialog(777, prefix..'список команд', '{'..acolor1..'}Информация\n{'..acolor5..'}/msgk\n{'..acolor5..'}/msg1\n{'..acolor5..'}/msg2\n{'..acolor5..'}/msg3\n{'..acolor5..'}/msg4\n{'..acolor5..'}/msg5\n{'..acolor5..'}/cmd1\n{'..acolor5..'}/cmd2\n{'..acolor5..'}/cmd3\n{'..acolor5..'}/getip\n{'..acolor5..'}/fcoins >> 2\n{'..acolor5..'}/otbor', 'Выбрать', 'Закрыть', 2)
end
function dialog()
	while true do
		wait(0)
		imgui.Process = imgbase.v
		local result, button, list, _ = sampHasDialogRespond(666)
		if result then if button == 1 then
			imgbase.v = true
			if list == 1 then img = 'amute'
			elseif list == 2 then img = 'autorep'
			elseif list == 3 then img = 'fast nicknames'
			elseif list == 4 then img = 'FChecker'
			elseif list == 5 then img = 'spam'
			elseif list == 6 then img = 'iznas'
			elseif list == 7 then img = 'asms'
			else img = 'info'
			end
		end end
		local result, button, list, _ = sampHasDialogRespond(777)
		if result then if button == 1 then
			imgbase.v = true
			if list == 1 then img = 'msgk'
			elseif list == 2 then img = 'msg1'
			elseif list == 3 then img = 'msg2'
			elseif list == 4 then img = 'msg3'
			elseif list == 5 then img = 'msg4'
			elseif list == 6 then img = 'msg5'
			elseif list == 7 then img = 'cmd1'
			elseif list == 8 then img = 'cmd2'
			elseif list == 9 then img = 'cmd3'
			elseif list == 10 then img = 'IP helper'
			elseif list == 11 then img = 'fast fcoins'
			elseif list == 12 then img = 'otbor'
			else img = 'info'
			end
		end end
