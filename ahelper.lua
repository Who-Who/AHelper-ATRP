...
function main()
	...
	local a = false
	for i = 1, #json.nicks do
		if nick == json.nicks[i] then
			a = true
			break
		end
	end
	if not a then
		sampAddChatMessage(prefix..'произошла непредвиденная ошибка. Код ошибки: '..acolor1..'666.', color3)
		sampAddChatMessage(prefix..'напиши мне '..acolor1..'(vk.com/jo_lac)'..acolor3..' об этой ошибке.', color3)
	...
	if imgs.menu.v then
		if imgs.base.v or imgs.info.v or imgs.msgk.v or imgs.msg.v or imgs.cmd.v or imgs.amute.v or imgs.autorep.v or imgs.fastnicknames.v or imgs.FChecker.v or imgs.IPhelper.v or imgs.spam.v or imgs.fastfcoins.v or imgs.iznas.v or imgs.asms.v or imgs.otbor.v then
			...
			if imgs.info.v then
				...
				imgui.Text(u8'/helper - список возможностей хелпера (без команд)')
				imgui.Text(u8'/cmds - список остальных возможностей хелпера')
				imgui.Text(u8'/checkupd - проверить обновления хелпера')
				imgui.Text(u8'/chmode - поменять отображение (ImGui >> SAMP диалоги)')
				imgui.Text('')
				imgui.End()
			end
			if imgs.msgk.v then
			...
			if imgs.FChecker.v then
				...
				imgui.BeginChild(prefix, imgui.ImVec2(345, 180), true)
					...
					imgui.Text(u8'/fcturn    - включение/выключение отображения ников.')
					imgui.Text(u8'/fcmove  - переместить окно отображения ников')
					imgui.Text(u8'/fcadd	 - добавить друга')
					imgui.Text(u8'/fcdelete - удалить друга')
...
function event.onServerMessage(color, text)
	--[[AUTOUPDATE-WAITER]]if text:find('^Вы вошли как .+') and text ~= 'Вы вошли как блогер' then
		lua_thread.create(
		function()
			sampSendChat('/agm')
			wait(1100)
...
function cmdFriendDelete(params)
	if params:find('%w+_%w+') then
		...
		sampAddChatMessage(prefix..'ник '..params..' успешно удалён.', color5)
		LIP.save('moonloader\\config\\FChecker.ini', data);
		thisScript():reload()
	else
...
function cmdFriendsChange(params)
	if params:find('%w+_%w+ %w+_%w+') then
		...
		sampAddChatMessage(prefix..'используй "/fcchange [прошлый ник] [новый ник]" (работают только ники формата Nick_Name)', color1)
	end
end
...
function save(name)
	...
	f:close()
	f = io.open('moonloader\\config\\'..name..'.json', 'r')
			...
			if doesFileExist(ajson) then
				...
				if f then
					...
					if updateversion ~= thisScript().version then
						sampShowDialog(900, prefix, acolor5..'Обнаружена новая версия: '..updateversion..'\nНововведения:'..inform, 'Обновить', 'Не обновлять', 0)
...
function autoupdate2()
	...
	function(id3, status1, p13, p23)
		...
		elseif status1 == dlstatus.STATUS_ENDDOWNLOADDATA then
			...
			if deljson then os.remove(getWorkingDirectory()..'\\config\\ahelper.json') sampAddChatMessage(prefix..'автоматически удалён JSON.', color5) else sampAddChatMessage(prefix..'JSON в этом обновлении удалять не надо.', color5) end
			goupdatestatus = true
			lua_thread.create(function() wait(500) thisScript():reload() end)
		end
		if status1 == dlstatus.STATUSEX_ENDDOWNLOAD then
			if goupdatestatus == nil then
				sampAddChatMessage((prefix..'Обновление прошло неудачно. Скорее всего, новая версия уже загружается на сервер.'), color5)
				sampAddChatMessage((prefix..'Потоврите попытку через несколько минут (/checkupd). Запускаю устаревшую версию..'), color5)
			end
		end
	end)
end
