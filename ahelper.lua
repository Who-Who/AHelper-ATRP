...
function imgui.OnDrawFrame()
	if imgbase.v then
		...
		elseif img == 'msgk' then
			...
			imgui.EndChild()
			imgui.Text('')
			imgui.Checkbox(u8'Вводить /fuelcars', imgmsgk.fuel)
			imgui.Checkbox(u8'Вводить /sduty', imgmsgk.sduty)
			if imgmsgk.fuel.v ~= json[img][1] then
				json[img][1] = imgmsgk.fuel.v
				save()
			end
			if imgmsgk.sduty.v ~= json[img][2] then
				json[img][2] = imgmsgk.sduty.v
				save()
			end
		...
		elseif img:find('cmd%d') then
			...
			if itba.v ~= nil and itba.v:find('%d+') and #itbb >= tonumber(itba.v) then
				for i = 1, tonumber(itba.v) do
					itbb[i].v = json[img][3][i]
					if imgui.InputText(u8'Строка', itbb[i]) then
						json[img][3][i] = u8:decode(itbb[i].v)
					end
					itbba[i].v = json[img][4][i]
					if imgui.InputText(u8'Задержка перед след строкой', itbba[i]) then
...
function waits(text, param, time)
	...
	for i, v in ipairs(text) do
		...
		elseif param:find('cmd%d+') then if #text ~= 0 then
				...
				sampAddChatMessage(prefix..'введи текст сообщения!', color1)
			end
		end
	end
